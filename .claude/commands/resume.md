...
Read `.claude/setup-progress.json` (in this repo).
...
Read `target_path` from the progress file. Display progress with a phase checklist:
...
╠══════════════════════════════════════════════════════════╣
║  Phases:                                                 ║
║   {✓ or ○} welcome          — Welcome & target path      ║
...
║   {✓ or ○} skills           — Skills from skills.sh     ║
...
║  Target: {target_path}                                   ║
║                                                          ║
║  WORKFLOW CHEATSHEET (run from target project):          ║
║                                                          ║
...
If "Continue": read `setup.md` (in this repo root) and resume from the first incomplete phase, using the `target_path` from the progress file.
...
/setup:init → (interrupted) → /setup:resume → continue
                                   ↓
                          (complete) → open target project → /flow:plan
```
